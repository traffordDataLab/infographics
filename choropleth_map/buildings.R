# IMD 2015 : choropleth map (buildings) ##
...
# intersect data  ---------------------------
bldgs_lsoa <- st_intersection(bldgs, lsoa)

# test results ---------------------------
plot(st_geometry(filter(lsoa, area_code == "E01006075")), col = "#eeeeee")
plot(st_geometry(filter(bldgs_lsoa, area_code == "E01006075")), col = "#D6437A", add = T)

# plot data  ---------------------------
ggplot() +
  geom_sf(data = bldgs_lsoa, aes(colour = factor(value)), fill = "#212121", alpha = 1, size = 0.1, show.legend = "line") +
  ...
  scale_colour_manual(breaks = 1:10,
                      ...
                      labels = c("Most \ndeprived", 2:9, "Least \n deprived")) +
  labs(x = NULL, y = NULL, 
       title = "Deprivation in Trafford by LSOA: buildings", 
...
# save plot  ---------------------------
ggsave(file = "IMD_2015_choropleth_map_buildings.svg", dpi = 300, width = 6, height = 6)
ggsave(file = "IMD_2015_choropleth_map_buildings.png", dpi = 300, width = 6, height = 6)
